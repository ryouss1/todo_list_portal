...
ステータス: **全項目対応済み（HIGH/MEDIUM: 2026-02-27、LOW: 3-1・3-3・3-4: 2026-02-27、3-2: 2026-02-28）**
...
**対応内容（2026-02-27）:**
- `asyncio.wait_for(websocket.receive_text(), timeout=WS_PING_INTERVAL)` でアイドルタイムアウトを設定
- タイムアウト時: サーバーから `__ping__` を `WS_PING_TIMEOUT` 秒以内に送信を試みる。送信失敗時はゾンビ接続と判定して `break`→`finally: disconnect()`
- クライアントからの `__ping__` には `__pong__` で応答（クライアント主導ハートビートにも対応）
- 設定: `WS_PING_INTERVAL`（デフォルト30秒）、`WS_PING_TIMEOUT`（デフォルト10秒）を `portal_core/portal_core/config.py` に追加。環境変数で上書き可能
- `WebSocketDisconnect` / ゾンビ切断どちらのパスでも `finally` ブロックで確実に `disconnect()` を実行
- テスト追加: `portal_core/tests/test_websocket.py` に `TestWebSocketHeartbeat` クラス（3件）追加

**残存する技術的負債:**
- ハンドラーループのゾンビ検出パス（`send_text` 失敗 → `break`）の完全なエンドツーエンドテストは TestClient の同期的性質上困難なため未実施

---

### 3-2. Excel エクスポートのメモリ使用 ✅ 対応済み（実用的最大限）

**ファイル:** `app/services/attendance_service.py`, `app/routers/api_attendances.py`

Excel ファイル生成時にファイル全体を `BytesIO` でメモリに保持する。同時に複数ユーザーが大量データのエクスポートを実行すると、メモリ消費が急増する。

**対応内容:**
- `app/routers/api_attendances.py` は既に `StreamingResponse(output, ...)` でレスポンスを返している（BytesIO をクライアントにストリーミング）。
- `openpyxl` はファイル全体をメモリ上に構築してから保存する設計のため、真のストリーミング生成は不可能。`BytesIO` への一括生成は不可避。
- 真のストリーミング（行単位での逐次生成）が必要な場合は `xlsxwriter` ライブラリへの切り替えが必要だが、改修コストが高く実運用上の件数（月次、1ユーザー分）では問題になりにくいため現状維持とする。

**残存リスク:** 大規模展開（多数ユーザーが同時に大量データをエクスポート）では依然としてメモリ消費増大の可能性あり。

---

### 3-3. `presence_service` の非アクティブユーザー混入 ✅ 対応済み
...
**対応内容（2026-02-27）:**
- `active_user_ids = {u.id for u in users}` でアクティブユーザーの ID セットを構築
- `status_map = {s.user_id: s for s in statuses if s.user_id in active_user_ids}` — 非アクティブユーザーの在籍レコードを `status_map` から除外
- テスト追加: `tests/test_presence.py` に `test_inactive_user_excluded_from_statuses` を追加（非アクティブユーザーが `GET /api/presence/statuses` レスポンスに含まれないことを検証）
...
| **LOW** | 3-2 | Excel メモリ使用 | ✅ 対応済み（StreamingResponse 使用中。openpyxl の仕様上、BytesIO 一括生成は不可避） |
