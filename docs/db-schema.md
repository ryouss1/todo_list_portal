...
logs（独立テーブル）
log_sources（独立テーブル）
attendance_presets（独立テーブル、users から FK 参照）
task_categories（独立テーブル、tasks/daily_reports/task_list_items から FK 参照）
groups（独立テーブル、users から FK 参照）
login_attempts（独立テーブル）
auth_audit_logs（users から FK 参照）
oauth_providers（独立テーブル）
oauth_states（独立テーブル）
```
...
| name | String(200) | NOT NULL | ソース名 |
| file_path | String(1000) | NOT NULL | 監視対象ファイルパス |
| system_name | String(200) | NOT NULL | logs.system_name に設定する値 |
| log_type | String(100) | NOT NULL | logs.log_type に設定する値 |
| parser_pattern | Text | NULL許可 | 正規表現（名前付きグループ） |
| severity_field | String(100) | NULL許可 | severity を抽出するグループ名 |
| default_severity | String(20) | DEFAULT "INFO" | severity 未抽出時のデフォルト |
| polling_interval_sec | Integer | DEFAULT 30 | ポーリング間隔（秒、最低5） |
| is_enabled | Boolean | DEFAULT true | 有効/無効 |
| last_read_position | BigInteger | DEFAULT 0 | ファイル読取位置 |
| last_file_size | BigInteger | DEFAULT 0 | 前回ファイルサイズ（ローテーション検出用） |
| last_collected_at | DateTime(TZ) | NULL許可 | 最終収集日時 |
...
> **注意**: Group および Calendar 系モデルは `app/models/__init__.py` に未登録。マイグレーションは適用済みだが、ルーターの `main.py` への組み込みは未完了。
