...
未来のタスクを含むバックログ管理や、担当者割り当ての機能がなかった。
...
| メソッド | パス | 説明 | レスポンス |
|---------|------|------|-----------|
| GET | `/api/task-list/all` | 全アイテム一覧（担当者/ステータスフィルタ対応） | `200` TaskListItemResponse[] |
| GET | `/api/task-list/unassigned` | 未割当アイテム一覧 | `200` TaskListItemResponse[] |
| GET | `/api/task-list/mine` | 自分に割り当てられたアイテム一覧（ステータスフィルタ対応） | `200` TaskListItemResponse[] |

**`GET /all` クエリパラメータ**:
- `assignee_id` (任意): 指定時 → そのユーザーのアイテムのみ。`0` → 未割当のみ。省略 → 全件
- `status` (任意, 複数指定可): 指定ステータスのみ返却。例: `?status=open&status=in_progress`
...
Task List ページ
├── [自分 (Mine)] タブ ← デフォルト
│   ├── フィルタバー（Status / Category / Keyword / Show Done）
│   └── テーブル一覧（自分にアサインされたアイテムのみ）
└── [全体 (All)] タブ
    ├── フィルタバー（同上）
    └── テーブル一覧（全アイテム表示、Assignee 列あり）
```

### 6.2 タブ構成

| タブ | API | Assignee列 | Start ボタン | 説明 |
|------|-----|-----------|-------------|------|
| 自分 (Mine) | `GET /api/task-list/mine` | 非表示 | **表示** | 自分にアサインされたアイテム |
| 全体 (All) | `GET /api/task-list/all` | 表示 | 条件付き表示 | 全アイテム（全員閲覧可） |

### 6.2.1 フィルタバー

タブとテーブルの間に配置。ハイブリッド方式（サーバーサイド + クライアントサイド）。

| フィルタ | 層 | UI 部品 | 動作 |
|---------|------|---------|------|
| Show Done | サーバー | チェックボックス | OFF(デフォルト): `?status=open&status=in_progress` でAPI取得。ON: 全件取得 |
| ステータス | クライアント | ボタングループ (All/Open/In Progress/Done) | Done ボタンは Show Done ON 時のみ表示 |
| カテゴリ | クライアント | ドロップダウン | category_id で即時フィルタ |
| キーワード | クライアント | テキスト入力 (300msデバウンス) | title, backlog_ticket_id の部分一致（大文字小文字無視） |

フィルタ状態はタブ切り替え時も維持される。
...
| `tests/test_task_list.py` | テスト（39 件） |
...
`tests/test_task_list.py` に 39 テストケース。

- **CRUD テスト（8 件）**: 作成・取得・更新・削除、全フィールド作成、バリデーション
- **アサインテスト（4 件）**: assign/unassign、他ユーザーアサイン済拒否
- **Start テスト（3 件）**: Task 作成、status 変更、重複ブロック
- **時間蓄積テスト（2 件）**: Done 時蓄積、アイテム保持
- **ステータス同期テスト（2 件）**: 自動リセット、部分完了
- **全件一覧テスト（3 件）**: /all フィルタ（全件、assignee_id、未割当）
- **権限テスト（6 件）**: 編集・Start・Unassign の権限チェック
- **重複制限テスト（3 件）**: 同一アイテム制限、異なるアイテム許可、Done 後再 Start
- **フィルタテスト（5 件）**: /mine /all の status パラメータ（後方互換、done 除外、done のみ、open のみ、active のみ）
- **ページアクセステスト（1 件）**: `/task-list` ルート
