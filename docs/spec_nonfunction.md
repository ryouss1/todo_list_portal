...
| `tests/test_attendances.py` | Attendance API（breaks + presets + input_type + 手動入力 + 月別フィルタ） | 52件 |
...
| `tests/test_reports.py` | Report API + 認可（category + time_minutes） | 17件 |
| `tests/test_summary.py` | Summary API（category trends + daily） | 12件 |
| `tests/test_log_sources.py` | Log Source API（パス検証 + RBAC） | 16件 |
| `tests/test_log_collector.py` | Log Collector ロジック（partial line含む） | 9件 |
| `tests/test_alerts.py` | Alert API（severity検証 + RBAC） | 16件 |
| `tests/test_alert_rules.py` | Alert Rule API + 評価エンジン（条件検証 + RBAC） | 29件 |
| `tests/test_task_categories.py` | Task Category API（RBAC含む） | 11件 |
| `tests/test_task_list.py` | Task List API（assignment + start + time蓄積 + status同期） | 32件 |
...
# 特定テストファイルのケース確認
pytest tests/test_auth.py --collect-only -q
```
