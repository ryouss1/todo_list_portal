...
| `portal_core/tests/test_rbac_models.py` | RBAC モデル（Role, RolePermission, UserRole, Menu, RoleMenu, UserMenu） | 6件 |
| `portal_core/tests/test_rbac_schemas.py` | RBAC スキーマ（RoleCreate/Response/Update, MenuCreate/Response, PermissionItem） | 6件 |
| `portal_core/tests/test_roles_api.py` | Roles API（CRUD + 権限設定 + ユーザーロール付与/剥奪 + RBAC制御） | 14件 |
| `portal_core/tests/test_menus_api.py` | Menus API（CRUD + my メニュー取得 + role/dept/user/自己サービス visibility 設定） | 15件 |
| `portal_core/tests/test_crud_roles.py` | CRUD roles（ジェネリック CRUD + 権限一括更新） | 10件 |
| `portal_core/tests/test_crud_menus.py` | CRUD menus（ジェネリック CRUD + ユーザー権限フィルタ + role/dept/user visibility CRUD） | 15件 |
| `portal_core/tests/test_nav_filtering.py` | ナビゲーション権限フィルタリング（role_menus/user_menus/department_menus による表示制御） | 8件 |
| `portal_core/tests/test_require_permission.py` | `require_permission()` 依存性注入（許可/拒否/未認証） | 3件 |
| `portal_core/tests/test_admin_ui_pages.py` | 管理 UI ページ（/roles, /menus ページアクセス制御） | 4件 |
| `portal_core/tests/test_seed_rbac.py` | RBAC 初期データシード（デフォルトロール・メニュー登録） | 3件 |
| `portal_core/tests/test_nav_cache.py` | ナビメニュー TTL キャッシュ（初期化・同一ユーザーキャッシュ・別ユーザー分離・TTL期限切れ・キャッシュ無効化） | 6件 |
| `portal_core/tests/test_rbac.py` | `require_admin` 依存性 + RBAC 権限チェック（許可/拒否/ワイルドカード） | 6件 |
| `portal_core/tests/test_app_factory.py` | PortalApp ファクトリ（nav item 登録 + setup_core + nav_items 一元管理） | 4件 |
| **コア小計** | | **279件** |
...
| `tests/test_log_sources.py` | Log Source API（CRUD + 複数パス + 認証情報マスク + バリデーション + RBAC + 接続テスト + ファイル一覧 + スキャン + アラート連携 + フォルダリンク + コンテンツ読み込み + 再読込 + サーキットブレーカー + ログエントリ検索） | 111件 |
...
| `tests/test_reminder_checker.py` | カレンダーリマインダーチェッカー（start/stop + ポーリング判定 + ウォッチドッグ） | 5件 |
| `tests/test_jobs.py` | GET /api/jobs/status（ジョブ一覧 + 構造検証 + 非管理者 403） | 3件 |
| **アプリ小計** | | **615件** |
