...
| `portal_core/tests/test_menus_api.py` | Menus API（CRUD + my メニュー取得） | 6件 |
| `portal_core/tests/test_crud_roles.py` | CRUD roles（ジェネリック CRUD + 権限一括更新） | 10件 |
| `portal_core/tests/test_crud_menus.py` | CRUD menus（ジェネリック CRUD + ユーザー権限フィルタ） | 7件 |
| `portal_core/tests/test_nav_filtering.py` | ナビゲーション権限フィルタリング（role_menus/user_menus による表示制御） | 6件 |
...
| `portal_core/tests/test_nav_cache.py` | ナビメニュー TTL キャッシュ（初期化・同一ユーザーキャッシュ・別ユーザー分離・TTL期限切れ） | 4件 |
| **コア小計** | | **248件** |
