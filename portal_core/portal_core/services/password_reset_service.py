...
from portal_core.crud import password_reset_token as crud_token
from portal_core.crud import user as crud_user
from portal_core.services import email_service
...
def request_password_reset(
    ...
    4. Commit, then send email (via background task if add_background_task is provided)

    Args:
        add_background_task: Optional callable (e.g. BackgroundTasks.add_task) for
            scheduling email sending asynchronously. When None, email is sent
            synchronously.
    """
    ...
    if add_background_task is not None:
        add_background_task(_send_reset_email, email, reset_url)
    else:
